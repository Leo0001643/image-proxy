...
#
# 产物:
#   deploy/bin/image-proxy        linux/amd64 可执行(后端)
...
# ---- step1: 交叉编译 image-proxy ----
echo "[build-local] step1 = cross-build image-proxy (linux/amd64)"
mkdir -p deploy/bin
...
ls -lh deploy/bin/image-proxy deploy/bin/goose web/dist/index.html
